...
def get_mal_id_from_title(title: str, season: int) -> Optional[int]:
    ...
    try:
        ...
        logging.debug("Found MAL ID: %s for %s", anime_id, json.dumps(best_match, indent=4))
...
def build_flags(anime_id: str, episode: int, chapters_file: str) -> str:
    ...
    try:
        ...
        if response.status_code != 200:
            logging.info("Failed to fetch AniSkip data (Status: %d)", response.status_code)
            return ""
        
        metadata = response.json()
        
        if not metadata.get("found"):
            logging.warning("No skip times found for anime %s episode %d", anime_id, episode)
            return ""
